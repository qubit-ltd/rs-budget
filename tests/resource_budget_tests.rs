...
fn accumulates_and_reports_remaining_capacity() {
    ...
    budget.try_consume(Kind::Nodes, 4).unwrap();
    assert_eq!(budget.used(), 4);
    assert_eq!(budget.remaining(), 6);
    assert!(!budget.is_empty());
    assert!(!budget.is_unused());
    budget.try_consume(Kind::Nodes, 6).unwrap();
    assert!(budget.is_empty());
}
...
fn failed_consumption_is_atomic() {
    ...
    budget.try_consume(Kind::Nodes, 8).unwrap();
    assert_eq!(
        budget.try_consume(Kind::Nodes, 3),
...
fn overflow_saturates_and_cannot_bypass_the_limit() {
    ...
    budget.try_consume(Kind::Nodes, usize::MAX - 2).unwrap();
    assert_eq!(
        budget.try_consume(Kind::Nodes, 10),
...
fn exhausting_consumption_clears_remaining_capacity() {
    ...
    budget.try_consume(Kind::Nodes, 8).unwrap();
    assert_eq!(
        budget.consume_or_exhaust(Kind::Nodes, 3),
        Err(LimitExceeded::new(Kind::Nodes, 10, 11)),
    );
...
fn release_restores_capacity() {
    let mut budget = ResourceLimit::new(10).budget();
    budget.try_consume(Kind::Nodes, 7).unwrap();
...
fn excessive_release_is_atomic() {
    let mut budget = ResourceLimit::new(10).budget();
    budget.try_consume(Kind::Nodes, 3).unwrap();
