...
fn test_decode_session_borrowing_reuses_caller_owned_budgets() {
    ...
    let mut value = JsonValueBudget::new(
        JsonValueLimits::empty()
            .with_payload_bytes_limit(ResourceLimit::new(JsonResource::PayloadBytes, 3_u64)),
    );
    {
        let mut session = JsonDecodeSession::borrowing(Some(&mut input), &mut value);
        decode_slice::<IgnoredAny, _>(br#"{"a":1}"#, &mut session)
