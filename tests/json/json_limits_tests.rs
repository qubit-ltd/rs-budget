...
fn test_json_limits_expose_configured_values() {
    ...
    assert_eq!(limits.max_input_bytes(), Some(6));
    assert_eq!(limits.max_output_bytes(), Some(7));
    assert_eq!(limits.max_string_bytes(), Some(8));
    assert_eq!(limits.max_number_bytes(), Some(9));
}

/// Verifies custom-resource JSON limits and their complete limit accessors.
#[test]
fn test_generic_json_limits_support_all_limit_dimensions() {
    ...
    let structure = StructureLimits::<Resource, u8>::empty()
        .with_depth_limit(ResourceLimit::new(Resource::Depth, 1))
        .with_nodes_limit(ResourceLimit::new(Resource::Nodes, 2))
        .with_sequence_items_limit(ResourceLimit::new(
            Resource::SequenceItems,
            3,
        ))
        .with_map_entries_limit(ResourceLimit::new(Resource::MapEntries, 4))
        .with_key_bytes_limit(ResourceLimit::new(Resource::KeyBytes, 5));
    let limits = JsonLimits::<Resource, u8>::empty()
        .with_input_bytes_limit(ResourceLimit::new(Resource::Input, 6))
        .with_output_bytes_limit(ResourceLimit::new(Resource::Output, 7))
        .with_string_bytes_limit(ResourceLimit::new(Resource::String, 8))
        .with_number_bytes_limit(ResourceLimit::new(Resource::Number, 9))
        .with_structure_limits(structure);

    assert_eq!(JsonLimits::<Resource, u8>::default(), JsonLimits::empty());
    assert_eq!(limits.max_input_bytes(), Some(6));
    assert_eq!(limits.max_output_bytes(), Some(7));
    assert_eq!(limits.max_string_bytes(), Some(8));
    assert_eq!(limits.max_number_bytes(), Some(9));
    ...
    assert_eq!(limits.max_key_bytes(), Some(5));
    assert_eq!(
        limits.input_bytes_limit().unwrap().resource(),
        &Resource::Input
    );
    assert_eq!(
        limits.output_bytes_limit().unwrap().resource(),
        &Resource::Output
    );
    assert_eq!(
