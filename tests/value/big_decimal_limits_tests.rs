...
fn test_scale_is_checked_before_coefficient() {
    ...
    let limits = BigDecimalLimits::new()
        ...
        .with_coefficient_limits(
            BigIntegerLimits::new().with_significant_decimal_digits_limit(ResourceLimit::new(
                TestResource::Digits,
                1_u64,
            )),
        );
...
fn test_big_decimal_accessors_and_unconfigured_limits() {
    let coefficient = BigIntegerLimits::new()
        .with_magnitude_bits_limit(ResourceLimit::new(TestResource::Digits, 8_u64));
    let limits = BigDecimalLimits::new()
